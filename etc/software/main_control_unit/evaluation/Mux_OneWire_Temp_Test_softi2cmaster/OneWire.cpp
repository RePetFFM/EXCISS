...
#include "OneWire.h"
// #include <Wire.h> // we don't want this obe
//#include <I2C.h>      // but this one
#include "Arduino.h" // rh for delayMicroseconds()
...
uint8_t OneWire::readByte()
{
  ...
  */
  ...
  return busMaster.readByte(mAddress, 1u);
}
...
{
  // TODO: Shintaro device presence detection required
  /*
  begin();
...
void OneWire::deviceReset()
{
  ...
  */
  busMaster.writeByte(mAddress, DS2482_COMMAND_RESET);
}
...
void OneWire::setReadPointer(uint8_t readPointer)
{
  ...
  */
  ...
  busMaster.writeByte(mAddress, DS2482_COMMAND_SRP, readPointer);
}
...
// Churn until the busy bit in the status register is clear
uint8_t OneWire::isBusy()
{
  uint8_t status;

  status = readStatus() & DS2482_STATUS_BUSY;
...
void OneWire::writeConfig(uint8_t config)
{
  ...
  */
  ...
  busMaster.writeByte(mAddress, DS2482_COMMAND_WRITECONFIG, config | (~config)<<4);
...
uint8_t OneWire::wireReset()
{
  ...
  */
  ...
  busMaster.writeByte(mAddress, DS2482_COMMAND_RESETWIRE);
...
void OneWire::wireWriteByte(uint8_t data, uint8_t power)
{
  ...
  busMaster.writeByte(mAddress, DS2482_COMMAND_WRITEBYTE, data);
}
...
uint8_t OneWire::wireReadByte()
{
  ...
  */
  ...
  busMaster.writeByte(mAddress, DS2482_COMMAND_READBYTE);
  waitOnBusy();
  return readData();
}
...
void OneWire::wireWriteBit(uint8_t data, uint8_t power)
{
  ...
  busMaster.writeByte(mAddress, DS2482_COMMAND_SINGLEBIT, data ? 0x80 : 0x00);
}

void OneWire::wireWriteBitInstant(uint8_t data)
{
  busMaster.writeByte(mAddress, DS2482_COMMAND_SINGLEBIT, data ? 0x80 : 0x00);
}
...
uint8_t OneWire::wireSearch(uint8_t *address)
{
  ...
  for(uint8_t i=0;i<64;i++)
  {
    ...
    */
    busMaster.writeByte(mAddress, DS2482_COMMAND_TRIPLET, direction ? 0x80 : 0x00);
