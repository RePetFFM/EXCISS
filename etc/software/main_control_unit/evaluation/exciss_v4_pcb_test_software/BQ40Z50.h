...
public:
	...
	uint16_t maxError = 0xFFFF;

	uint16_t voltage = 0xFFFF;

	int16_t current = 0xFFFF;

	uint16_t batteryChemID = 0xFFFF;

	uint16_t remainingCurrent = 0xFFFF;
	
	uint16_t fullChargCapacity = 0xFFFF;
	
	uint16_t minToFull = 0xFFFF;
	
	uint16_t minToEmpty = 0xFFFF;

	uint16_t minRuntime = 0xFFFF;
	
	uint16_t temperature = 0xFFFF;

	uint16_t chargeRelative = 0xFFFF;

	uint16_t chargeAbsolute = 0xFFFF;

	uint16_t batteryStatus = 0xFFFF;
	
	uint16_t cycleCount = 0xFFFF;
	
	uint16_t cell1voltage = 0xFFFF;
	
	uint16_t cell2voltage = 0xFFFF;
	
	uint16_t cell3voltage = 0xFFFF;

	uint16_t cell4voltage = 0xFFFF;


	BQ40Z50 (uint8_t address);
