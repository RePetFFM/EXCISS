...
int charg_ok_previews_state = 0;
int shutdown_counter = 0;

void loop() {

  int charg_ok = digitalRead(DIN_PSU_CHRG_OK);

  if(charg_ok!=charg_ok_previews_state && charg_ok==0) {
    Serial.println("start shutdown");
    Serial3.println("start shutdown");  
    digitalWrite( DOUT_SCU_SHUTDOWN_REQUEST , STATE_SCU_SHUTDOWN_REQUEST );
  } 

  if(charg_ok==0) {
    shutdown_counter++;
    Serial.println(shutdown_counter);
    Serial3.println(shutdown_counter);  
  }
  
  charg_ok_previews_state = charg_ok;
  delay(1000);
}


void psu_debug() {
  Serial.println("//// charge ok");

  Serial.println( digitalRead( DIN_PSU_CHRG_OK ) );
  ...
  Serial.print("batteryChemID ");
  Serial.println(gauge.batteryChemID,HEX);
